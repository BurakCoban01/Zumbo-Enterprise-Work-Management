...
internal sealed class LabelMutationPipeline(
    ...
    IWorkItemAutomationChainContextAccessor? automationChain)
{
    ...
    internal async Task<WorkItemResponse> PersistAsync(
        ...
        string automationMutationType,
        string automationMutationValue,
        CancellationToken ct)
    {
        ...
        await PublishAutomationAsync(
            ...
            workItem.Status,
            MutationEventId(workItem, eventDiscriminator),
            $"{automationMutationType}:{workItem.Version}:{automationMutationValue}",
            ct);
