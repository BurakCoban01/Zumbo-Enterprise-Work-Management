...
internal sealed class CustomFieldMutationPipeline(
    ...
    IWorkItemAutomationChainContextAccessor? automationChain)
{
        ...
        CancellationToken ct)
    {
        ...
        await audit.WriteAsync(
            ...
            workItem.Id,
            oldValue,
            SerializeValues(workItem.CustomFields),
            correlationId,
            ct);
    ...
    internal static string SerializeValues(IEnumerable<WorkItemCustomFieldValueDocument> values) =>
        string.Join('|', values.Select(value => $"{value.FieldKey}:{value.SearchValue}"));
        ...
        CancellationToken ct)
    {
        ...
        await collaborationService.NotifyWatchersAsync(
            ...
            organizationId,
            "WatcherUpdate",
            $"{workItem.Title}: Custom fields updated",
            correlationId,
            null,
            ct);
    }
        ...
        CancellationToken ct)
    {
        ...
        return automationEvents.PublishAsync(
            new WorkItemAutomationEvent(
                ...
                workItem.ProjectId,
                "WorkItemUpdated",
                $"{workItem.Id}:assigned:{workItem.Version}",
                workItem.Id,
