...
public static class SaveCapacityPlanValidator
{
    ...
    public static SaveCapacityPlanRequest Validate(SaveCapacityPlanRequest request)
    {
        if (request.PeriodEnd < request.PeriodStart)
        {
            throw new ValidationException(
                "Capacity-plan end date must be after start date.");
        }

        if (request.PeriodEnd.DayNumber - request.PeriodStart.DayNumber + 1 > 366)
        {
            throw new ValidationException(
                "Capacity-plan period cannot exceed 366 days.");
        }

        var projectIds = NormalizeIds(
            request.ProjectIds
                ?? throw new ValidationException(
                    "Capacity-plan project list is required."),
            MaximumProjects,
            "Capacity-plan project");
        if (projectIds.Count == 0)
        {
            throw new ValidationException(
                "Capacity plan must include at least one project.");
        }
        ...
        var requestedMembers = request.Members
            ?? throw new ValidationException(
                "Capacity-plan member list is required.");
        ...
        var members = requestedMembers.Select(member =>
        {
            ...
            {
                throw new ValidationException(
                    "Capacity-plan member users must be unique.");
            }

            if (member.WeeklyCapacityHours is < 0 or > 168)
            {
                throw new ValidationException(
                    "Weekly capacity must be between 0 and 168 hours.");
            }
        ...
        var allocations = NormalizeAllocations(
            request.Allocations
                ?? throw new ValidationException(
                    "Capacity-plan allocation list is required."),
            memberIds,
        ...
        string label)
    {
        ...
        {
            throw new ValidationException(
                $"{label} list cannot exceed {maximum} entries.");
        }
    ...
    private static string? Optional(string? value, int maximum)
    {
        ...
        {
            throw new ValidationException(
                $"Value cannot exceed {maximum} characters.");
        }
