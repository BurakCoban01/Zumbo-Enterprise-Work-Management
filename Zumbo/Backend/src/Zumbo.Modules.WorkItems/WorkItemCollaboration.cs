...
public sealed record WorkItemCollaborationResponse(
    ...
    bool Voted,
    long Version);
public sealed record SetWorkItemWatchRequest(bool Watching);
public sealed record SetWorkItemVoteRequest(bool Voted);

public sealed record WorkItemEventActivityResponse(
    string Id,
    string Type,
    string ActorUserId,
    string Detail,
...
public sealed record WorkItemEventActivityPage(
    IReadOnlyCollection<WorkItemEventActivityResponse> Items,
    int Page,
    int PageSize,
    long TotalCount);
