...
public sealed class ArchitectureBoundaryTests
{
    ...
    public void OpenSearchWorkItemSearchAdapter_IsGroupedByResponsibility()
    {
        ...
        var forwardedTypes = Assembly.Load("Zumbo.BuildingBlocks.Infrastructure")
            .GetForwardedTypes()
            .OrderBy(type => type.FullName, StringComparer.Ordinal)
            .ToArray();
        Assert.Equal(
            [
                "Zumbo.BuildingBlocks.Infrastructure.Search.InMemoryWorkItemSearchIndex",
                "Zumbo.BuildingBlocks.Infrastructure.Search.OpenSearchOptions",
                "Zumbo.BuildingBlocks.Infrastructure.Search.OpenSearchWorkItemSearchIndex"
            ],
            forwardedTypes.Select(type => type.FullName!).ToArray());
        Assert.All(
            forwardedTypes,
            type => Assert.Equal("Zumbo.Modules.WorkItems", type.Assembly.GetName().Name));
    }

    [Fact]
    public void WorkItemSearchContracts_AreOwnedByWorkItemsContractsAssembly()
    {
        var contractDirectory = Path.Combine(SourceDirectory, "Zumbo.Modules.WorkItems.Contracts", "Search");
        Assert.Equal(
            [
                "IWorkItemSearchIndex.cs",
                "SearchOptions.cs",
                "WorkItemSearchQuery.cs",
                "WorkItemSearchRebuildResult.cs",
                "WorkItemSearchRecord.cs",
                "WorkItemSearchResult.cs",
                "WorkItemSearchUnavailableException.cs"
            ],
            Directory.GetFiles(contractDirectory, "*.cs", SearchOption.TopDirectoryOnly)
                .Select(path => Path.GetFileName(path)!)
                .Order(StringComparer.Ordinal)
                .ToArray());
        ...
        var forwardedTypes = Assembly.Load("Zumbo.BuildingBlocks.Application")
            .GetForwardedTypes()
            .Where(type => type.Namespace == "Zumbo.BuildingBlocks.Application.Search")
            .OrderBy(type => type.FullName, StringComparer.Ordinal)
            .ToArray();
        Assert.Equal(
            [
                "Zumbo.BuildingBlocks.Application.Search.IWorkItemSearchIndex",
                "Zumbo.BuildingBlocks.Application.Search.SearchOptions",
                "Zumbo.BuildingBlocks.Application.Search.WorkItemSearchQuery",
                "Zumbo.BuildingBlocks.Application.Search.WorkItemSearchRebuildResult",
                "Zumbo.BuildingBlocks.Application.Search.WorkItemSearchRecord",
                "Zumbo.BuildingBlocks.Application.Search.WorkItemSearchResult",
                "Zumbo.BuildingBlocks.Application.Search.WorkItemSearchUnavailableException"
            ],
            forwardedTypes.Select(type => type.FullName!).ToArray());
        Assert.All(
            forwardedTypes,
            type => Assert.Equal("Zumbo.Modules.WorkItems.Contracts", type.Assembly.GetName().Name));
    }

    [Fact]
    public void IdentitySecurityContracts_AreOwnedByIdentityContractsAssembly()
    {
        var contractDirectory = Path.Combine(SourceDirectory, "Zumbo.Modules.Identity.Contracts", "Security");
        Assert.Equal(
            [
                "ApiKeyScopes.cs",
                "IPasswordHasher.cs",
        ...
        var legacyContractDirectory = Path.Combine(
            SourceDirectory,
            "Zumbo.BuildingBlocks.Application",
            "Security",
            "Contracts",
            "IdentitySecurity");
        Assert.Empty(
            Directory.Exists(legacyContractDirectory)
                ? Directory.GetFiles(legacyContractDirectory, "*.cs", SearchOption.AllDirectories)
                : []);
        ...
        var forwardedTypes = Assembly.Load("Zumbo.BuildingBlocks.Application")
            .GetForwardedTypes()
            .Where(type => type.Assembly.GetName().Name == "Zumbo.Modules.Identity.Contracts")
            .OrderBy(type => type.FullName, StringComparer.Ordinal)
            .ToArray();
        Assert.Equal(
            [
                "Zumbo.BuildingBlocks.Application.Security.ApiKeyScopes",
                "Zumbo.BuildingBlocks.Application.Security.IPasswordHasher",
                "Zumbo.BuildingBlocks.Application.Security.ITokenIssuer",
                "Zumbo.BuildingBlocks.Application.Security.JwtOptions",
                "Zumbo.BuildingBlocks.Application.Security.PermissionCatalog",
                "Zumbo.BuildingBlocks.Application.Security.TokenUser"
            ],
            forwardedTypes.Select(type => type.FullName!).ToArray());
    }

    [Fact]
    public void ProjectResourcePolicyContracts_AreOwnedByProjectsContractsAssembly()
    {
        var contractDirectory = Path.Combine(SourceDirectory, "Zumbo.Modules.Projects.Contracts", "Security");
        Assert.Equal(
            [
                "IProjectResourcePolicy.cs",
                "ProjectResourceAuthorization.cs"
            ],
            Directory.GetFiles(contractDirectory, "*.cs", SearchOption.TopDirectoryOnly)
                .Select(path => Path.GetFileName(path)!)
                .Order(StringComparer.Ordinal)
        ...
        var forwardedTypes = Assembly.Load("Zumbo.BuildingBlocks.Application")
            .GetForwardedTypes()
            .Where(type => type.Assembly.GetName().Name == "Zumbo.Modules.Projects.Contracts")
            .OrderBy(type => type.FullName, StringComparer.Ordinal)
            .ToArray();
        Assert.Equal(
            [
                "Zumbo.BuildingBlocks.Application.Security.IProjectResourcePolicy",
                "Zumbo.BuildingBlocks.Application.Security.ProjectResourceAuthorization"
            ],
            forwardedTypes.Select(type => type.FullName!).ToArray());
    }
    ...
    public void ModuleProjectReferences_MatchExplicitAllowedContractGraph()
    {
        var expected = new[]
        {
            ...
            "Zumbo.Modules.Identity->Zumbo.Modules.Identity.Contracts",
            ...
            "Zumbo.Modules.WorkItems->Zumbo.Modules.Identity.Contracts",
            "Zumbo.Modules.WorkItems->Zumbo.Modules.Projects.Contracts",
            "Zumbo.Modules.WorkItems->Zumbo.Modules.WorkItems.Contracts",
    ...
    [Fact]
    public void WorkItemComments_AreOwnedByFocusedController()
    {
        var controller = File.ReadAllText(Path.Combine(
            ...
            "WorkItems",
            "Comments",
            "WorkItemCommentsController.cs"));
        Assert.Contains("namespace Zumbo.Api.Presentation.Controllers.WorkItems.Comments;", controller, StringComparison.Ordinal);
        Assert.Contains("public sealed class WorkItemCommentsController : ApiControllerBase", controller, StringComparison.Ordinal);
        Assert.Contains("[Route(\"/api/work-items/{id}/comments\")]", controller, StringComparison.Ordinal);
        ...
        Assert.False(File.Exists(Path.Combine(obsoleteDirectory, "DeleteCommentEndpoint.cs")));
        Assert.False(File.Exists(Path.Combine(obsoleteDirectory, "ListCommentsEndpoint.cs")));
        Assert.False(File.Exists(Path.Combine(obsoleteDirectory, "ListCommentRevisionsEndpoint.cs")));
    }

    [Fact]
    public void WorkItemLabels_AreOwnedByFocusedController()
    {
        AssertFocusedWorkItemController("Labels", "WorkItemLabelsController.cs", "WorkItemLabelsController", "AddLabelEndpoint.Map(group);", "RemoveLabelEndpoint.Map(group);");
    }

    [Fact]
    public void WorkItemChecklist_IsOwnedByFocusedController()
    {
        AssertFocusedWorkItemController("Checklist", "WorkItemChecklistController.cs", "WorkItemChecklistController", "AddChecklistItemEndpoint.Map(group);", "SetChecklistItemCompletionEndpoint.Map(group);");
    }

    [Fact]
    public void WorkItemRelations_AreOwnedByFocusedController()
    {
        AssertFocusedWorkItemController("Relations", "WorkItemRelationsController.cs", "WorkItemRelationsController", "LinkWorkItemEndpoint.Map(group);", "UnlinkWorkItemEndpoint.Map(group);");
    }

    [Fact]
    public void WorkItemWorklogs_AreOwnedByFocusedController()
    {
        AssertFocusedWorkItemController("Worklogs", "WorkItemWorklogsController.cs", "WorkItemWorklogsController", "AddWorkLogEndpoint.Map(group);", "ListWorkLogsEndpoint.Map(group);");
    }
    ...
    [Fact]
    public void WorkItemApprovals_AreOwnedByFocusedController()
    {
        AssertFocusedWorkItemController("Approvals", "WorkItemApprovalsController.cs", "WorkItemApprovalsController", "RequestApprovalEndpoint.Map(group);", "DecideApprovalEndpoint.Map(group);", "ListApprovalsEndpoint.Map(group);");
    }

    private static void AssertFocusedWorkItemController(
        string feature,
        string fileName,
        string className,
        params string[] obsoleteMaps)
    {
        var source = File.ReadAllText(Path.Combine(
            ...
            feature,
            fileName));
        Assert.Contains($"namespace Zumbo.Api.Presentation.Controllers.WorkItems.{feature};", source, StringComparison.Ordinal);
    ...
    public void WorkItemEndpointHost_IsRemovedAfterControllerClosure()
    {
        ...
        var removedCompatibilityFiles = new[]
        {
            ...
            "WorkItemEndpoints.BulkOperations.cs", "WorkItemEndpoints.Checklist.cs", "WorkItemEndpoints.Comments.cs",
            "WorkItemEndpoints.DurableMessaging.cs", "WorkItemEndpoints.Labels.cs", "WorkItemEndpoints.Planning.cs",
            "WorkItemEndpoints.Realtime.cs", "WorkItemEndpoints.Recurrences.cs", "WorkItemEndpoints.Relations.cs",
    ...
    [Fact]
    public void WorkItemCollaboration_IsOwnedByFocusedController()
    {
        var controller = File.ReadAllText(Path.Combine(SourceDirectory, "Zumbo.Api", "Presentation", "Controllers", "WorkItems", "Collaboration", "WorkItemCollaborationController.cs"));
        Assert.Contains("public sealed class WorkItemCollaborationController : ApiControllerBase", controller, StringComparison.Ordinal);
        Assert.Contains("[HttpGet(\"collaboration\")]", controller, StringComparison.Ordinal);
        Assert.Contains("[HttpPut(\"watch\")]", controller, StringComparison.Ordinal);
    ...
    [Fact]
    public void WorkItemActivity_IsOwnedByFocusedController()
    {
        var controller = File.ReadAllText(Path.Combine(SourceDirectory, "Zumbo.Api", "Presentation", "Controllers", "WorkItems", "Activity", "WorkItemActivityController.cs"));
        Assert.Contains("public sealed class WorkItemActivityController : ApiControllerBase", controller, StringComparison.Ordinal);
