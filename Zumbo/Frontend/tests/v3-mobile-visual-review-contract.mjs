...
export function validateMobileVisualReview(matrix, review) {
  const issues = [];
  requireCondition(matrix?.taskId === 'V3-QA-002', 'Matrix taskId must be V3-QA-002.', issues);
  requireCondition(matrix?.status === 'Passed', 'Automated mobile matrix must pass before visual approval.', issues);
  requireCondition(
  ...
  if (issues.length) {
    const error = new Error(`Mobile visual review is not acceptable:\n${issues.join('\n')}`);
    error.issues = issues;
    throw error;
  }
  return {
