...
test('desktop surface exposes normal, read-only, public, triage and idempotent submission paths', () => {
  ...
  assert.match(desktopFeature, /var requestId = \+\+vm\.intakeLoadRequestId/);
  assert.match(desktopFeature, /requestId !== vm\.intakeLoadRequestId/);
  assert.match(desktopFeature, /requestId === vm\.intakeLoadRequestId/);
  assert.match(desktopFeature, /preserveNewDraft = vm\.intakeEditorOpen/);
  assert.match(desktopFeature, /if \(vm\.intakeLoading \|\| vm\.intakeBusy \|\| !vm\.boards\.length\) return;/);
  ...
  assert.match(desktopHtml, /ng-disabled="vm\.intakeLoading \|\| vm\.intakeBusy \|\| !vm\.boards\.length"/);
  assert.match(desktopHtml, /vm\.triageIntakeSubmission/);
  assert.match(desktopHtml, /class="public-intake-shell"/);
  assert.match(desktopHtml, /class="side-nav" ng-if="vm\.session\.currentUser && !vm\.publicIntakeId"/);
  assert.match(desktopHtml, /vm\.selectedTask && !vm\.publicIntakeId/);
  assert.match(desktopHtml, /\['overview','catalog','intake','automation','jobs'\]\.indexOf\(vm\.workMode\)/);
  assert.match(desktopCss, /grid-template-columns: minmax\(250px, 320px\) minmax\(0, 1fr\)/);
  assert.match(desktopCss, /\.intake-submission-form select,[\s\S]*min-height: 40px/);
  assert.match(desktopCss, /\.intake-submission-form textarea \{[\s\S]*min-height: 96px/);
  assert.match(desktopCss, /border: 1px solid var\(--line-strong\)/);
  assert.doesNotMatch(desktopCss, /var\(--border(?:-strong)?\)/);
...
test('mobile surface has protected project parity and an unprotected public form state', () => {
  ...
  assert.match(mobileHtml, /class="mobile-intake-honeypot" aria-hidden="true"/);
  assert.match(mobileCss, /\.mobile-intake-form \.mobile-intake-honeypot \{\s*display: none !important;/);
  assert.match(mobileCss, /\.mobile-intake-section > header > \.button \{\s*min-width: 44px;\s*min-height: 44px;/);
