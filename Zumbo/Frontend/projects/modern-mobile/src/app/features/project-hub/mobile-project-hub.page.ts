...
export class MobileProjectHubPage {
  private readonly route=inject(ActivatedRoute); private readonly api=inject(MobileProjectHubService); private readonly session=inject(ZumboSessionService); private readonly destroyRef=inject(DestroyRef);
  ...
  constructor(){addIcons({arrowBackOutline,arrowForwardOutline,calendarOutline,warningOutline}); const update=()=>this.online.set(navigator.onLine); window.addEventListener('online',update);window.addEventListener('offline',update);this.destroyRef.onDestroy(()=>{window.removeEventListener('online',update);window.removeEventListener('offline',update);});this.load();}
  protected selectTab(event:CustomEvent):void{this.tab.set((event.detail.value||'overview') as MobileProjectHubTab);}
  protected load(done?:()=>void):void{if(!this.projectId){done?.();return;}this.loading.set(true);this.error.set(null);void this.store.load().then(()=>{this.api.load(this.projectId).pipe(finalize(()=>{this.loading.set(false);done?.();})).subscribe({next:value=>{this.data.set(value);if(!this.statuses().some(item=>item.name===this.selectedStatus()))this.selectedStatus.set(this.statuses()[0]?.name||'');if(value.failures.length)this.error.set(`${value.failures.join(', ')} geçici olarak alınamadı.`);},error:()=>this.error.set('Proje merkezi yüklenemedi.')});}).catch(()=>{this.loading.set(false);this.error.set('Proje bulunamadı.');done?.();});}
