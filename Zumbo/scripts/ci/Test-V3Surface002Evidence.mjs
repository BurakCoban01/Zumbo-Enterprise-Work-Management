...
const evidence = json('artifacts/v3/V3-SURFACE-002.json');
const visual = json('artifacts/v3/V3-SURFACE-002-visual.json');
const deterministic = json('artifacts/ui/v3-surface-002/result.json');
const real = json('artifacts/ui/v3-surface-002-real/result.json');
const matrix = json('docs/product/api-ui-capability-matrix.json');

assert.equal(evidence.schemaVersion, 1);
assert.equal(evidence.task, 'V3-SURFACE-002');
assert.equal(evidence.passed, true);
assert.equal(evidence.noDeployment, true);
assert.equal(evidence.userChangesPreserved, true);
assert.deepEqual(evidence.validation.frontend.unit, { passed: 127, failed: 0, skipped: 0 });
assert.equal(evidence.validation.frontend.build.assets, 73);
...
assert.equal(evidence.validation.backend.previewCountMinimum, 1);
assert.equal(evidence.validation.backend.previewCountMaximum, 10);
assert.equal(evidence.validation.backend.durableTemplateAndRecurrenceAudit, true);
...
assert.equal(evidence.validation.visualReview.mcpBrowserAvailable, false);
...
assert.equal(visual.task, 'V3-SURFACE-002');
...
assert.ok(matrix.summary.frontendCalls >= 290);
assert.ok(matrix.summary.desktopCalls >= 182);
assert.ok(matrix.summary.mobileCalls >= 108);
assert.equal(matrix.summary.duplicateOperations, 0);
assert.equal(matrix.summary.unmatchedFrontendCalls, 0);
assert.equal(matrix.summary.unownedOperations, 0);
assert.ok(matrix.informationArchitecture.desktop.projectViews.includes('automation'));
...
assert.equal(operation('POST /api/work-items/recurrences/preview').status, 'surfaced');
assert.equal(operation('POST /api/work-items/recurrences/process-due').targetSurface, 'desktop-admin/operations');

console.log('V3-SURFACE-002 evidence passed: authoritative preview, real scheduler/audit, 9 deterministic and 10 real checks, and 6 reviewed captures.');
