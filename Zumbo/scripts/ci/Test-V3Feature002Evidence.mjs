...
const evidence = json('artifacts/v3/V3-FEATURE-002.json');
const visual = json('artifacts/v3/V3-FEATURE-002-visual.json');

assert.equal(evidence.schemaVersion, 1);
assert.equal(evidence.task, 'V3-FEATURE-002');
...
assert.equal(evidence.validation.providerParity.postgreSql.passed, 4);
assert.equal(evidence.validation.browser.deterministic.checks, 9);
assert.equal(evidence.validation.browser.mcpRealApi.checks.length, 9);
...
assert.ok(Object.values(evidence.validation.automation).every(Boolean));
...
assert.equal(visual.task, 'V3-FEATURE-002');
...
console.log('V3-FEATURE-002 evidence passed: full automation, provider parity, 185 frontend tests and 12 browser/visual checks.');
